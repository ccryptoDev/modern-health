...
module.exports = {
  ...
  completeApplication: completeApplicationAction,
...
function ajaxPaymentHistoryAction(req, res){
	...
	.then(function(vikingData) {
		...
		vikingData.forEach(function(vikingInfo,loopvalue){
			...
			paymentHistory.push({ loopid:loopid,
					...
					else
					{
						...
						Account
						  ...
						  .then(function(accountDetail) {
							   ...
							   var accountuserbank='';
							     if ("undefined" !== typeof accountDetail && accountDetail!='' && accountDetail!=null)
								 {

									   accountuserbank = accountDetail[0].userBankAccount.id;
									   var available;
									   var  current;


									    //--Filled out manually check
									    var userbankDetails = accountDetail[0].userBankAccount;

										if(userbankDetails.bankfilloutmanually)
										{
										if(userbankDetails.bankfilloutmanually==1)
										{
											userfilloutmanually =1;
										}
										}
										else
										{
										  if ("undefined" === typeof userbankDetails.item_id || userbankDetails.item_id=='' || userbankDetails.item_id==null)
										  {
											  userfilloutmanually =1;
										  }
										}
										...
										_.forEach(accountDetail, function(accountvalue) {
											   available = parseFloat(accountvalue.balance.available).toFixed(2);
												if(available > 0)
												{
													accountvalue.balance.available = available.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
													accountvalue.balance.available = '$'+accountvalue.balance.available;
												}
												current = parseFloat(accountvalue.balance.current).toFixed(2);
												if(current > 0)
												{
													accountvalue.balance.current = current.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
													accountvalue.balance.current = '$'+accountvalue.balance.current;
												}
												if(accountvalue.accountNumber)
												{
												  var str = accountvalue.accountNumber;
												  accountvalue.accountNumber = str.replace(/\d(?=\d{4})/g, "X");
												  //Need to work for other accounts
												  var otheraccounts = accountvalue.userBankAccount.accounts;
												   var availables;
												   var  currents;
												  _.forEach(otheraccounts, function(otheraccountvalue) {
														availables = parseFloat(otheraccountvalue.balance.available).toFixed(2);
														if(availables > 0){
															otheraccountvalue.balance.available = availables.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
															otheraccountvalue.balance.available = '$'+otheraccountvalue.balance.available;
														}
														currents = parseFloat(otheraccountvalue.balance.current).toFixed(2);
														if(currents > 0){
															otheraccountvalue.balance.current = currents.toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
															otheraccountvalue.balance.current = '$'+otheraccountvalue.balance.current;
														}
							  ...
							  .then(function(documentdata) {
									  _.forEach(documentdata, function(documentvalue) {
										...
										var pdfCriteria = {paymentManagement: paymentmanagementdata.id};
										  ...
										  Logactivity
											...
											.then(function(logDetails) {
												 ...
												 .then(function(plaidDetails) {
													 ...
													 var transcriteria = { id: paymentmanagementdata.screentracking.transunion};
													 ...
													 .then(function (transData) {
														 ...
														 /* if(userAccount.product.subject.subjectRecord.custom.credit.trade)
														  ...
														  var screentrackingres = paymentmanagementdata.screentracking;
														 var monthlyIncomeAmount =  parseFloat(screentrackingres.incomeamount || "0");

														 var annualIncomeAmount = 0;
														  ...
														  Screentracking
															...
															.then(function(dtiandoffers) {
																...
																.then(function(repullDetails) {
																	   ...
																	   Makepayment
																		...
																		.then(function(makePaymentForStory) {
																		   ...
																		   if(paymentmanagementdata.localcreditfilepath!='' && paymentmanagementdata.localcreditfilepath!=null && "undefined" !== typeof paymentmanagementdata.localcreditfilepath)
																		   {
																			...
																			/*var vikingCriteria = {payment_id:paymentmanagementdata.id,userId:paymentmanagementdata.user};
																		var vikingCriteriaPending = {payment_id:paymentmanagementdata.id,userId:paymentmanagementdata.user,lenderType:'debit',processType:{$ne:1}};
																		VikingRequest.find(vikingCriteria).sort("scheduleDate").then(function (vikingData){
																			var vikingcreditData = _.filter(vikingData, { "lenderType": 'credit' });
																			var creditFileStatus = 'sent';
																			if(vikingcreditData[0]){
																				var vikingCreditAmt =  vikingcreditData[0].amount;
																				if(vikingcreditData[0].processType == 1){
																					creditFileStatus = 'pending';
																				}
																			}
																			VikingRequest.count(vikingCriteriaPending).exec(function countCB(error, totalrecords) {
																				var vikingResult = vikingData;
																				//sails.log("vikingCriteria",vikingData);
																				var IPFromRequest =  req.headers['x-forwarded-for'] || req.connection.remoteAddress;
																				var indexOfColon = IPFromRequest.lastIndexOf(':');
																				var ip = IPFromRequest.substring(indexOfColon+1,IPFromRequest.length);
																					  ...
																					  var responsedata = {
																						...
																						pendingSchedule:pendingSchedule,
																						paidSchedule:paidSchedule,
																						likersCount: paymentmanagementdata.story.likers.length,
																						dislikersCount: paymentmanagementdata.story.dislikers.length,
																						transData:transData,
																						shwotransData:showtransData,
																						accountDetails:subTypeArray,
																						totalbalance:totalbalance,
																   ...
																   .catch(function(err){
																		sails.log.error('AchController#getAchUserDetailsAction Repullsection :: Error :: ', err);
																		res.view("admin/error/404", {
																			data: err.message,
																			layout: 'layout'
																		 });
...
function completeApplicationAction(req, res){

	/*var options = {
		//status:'OPENED',
		$or : [ { status : 'OPENED' }, { status : 'PAID OFF' } ],
		//isPaymentActive: true,
		//achstatus: { $eq: [1,2], $exists: true },
		$or : [ { achstatus : 1,isPaymentActive: true }, { achstatus : 2,isPaymentActive: false } ],
    };*/
	...
	/*var options = {
		...
		//isPaymentActive: true,
		//achstatus: { $eq: [1,2], $exists: true },
		$or : [ { achstatus : 1 }, { achstatus : 2 } ],
		//$or : [ { achstatus : 1,isPaymentActive: true }, { achstatus : 2,isPaymentActive: false } ],
    };*/

	//var checktodaysDate = moment().startOf('day').format('MM-DD-YYYY');
	var  checktodaysDate 	 =	moment().tz("America/Los_Angeles").startOf('day').format('MM-DD-YYYY');
	if("undefined" !== req.param("viewtype") && req.param("viewtype")!='' && req.param("viewtype")!=null)
    {
		 var viewtype = req.param("viewtype");
    ...
    {
	 	var viewtype = 'approved';
    }
	//sails.log.info("viewtype:",viewtype);
	//sails.log.info("checktodaysDate:",checktodaysDate);
	...
	if ("undefined" !== typeof req.session.adminpracticeID && req.session.adminpracticeID!='' && req.session.adminpracticeID!=null)
	{
		...
		else if(viewtype=='progress')
		{
			var options = {
				...
				practicemanagement:req.session.adminpracticeID,
				$and :[
					   //{ loanSetdate : { $lte : new Date(checktodaysDate), $exists: true } },
					   { firstpaymentcompleted : { $eq : 1, $exists: true } },
					   { $or : [ { moveToArchive : { $exists: false} }, { moveToArchive:{ $eq: 0, $exists: true } }  ] }
					  ]
			};
		}
		...
		else if(viewtype=='archived')
		{
			var options = {
				$or : [ { status : 'PAID OFF' } ,
					    { status : 'CLOSED' } ,
						{ $and : [{ status : 'ARCHIVED' }, { moveToArchive:{ $eq: 1, $exists: true } }]}
				],
				isPaymentActive: true,
				achstatus: { $eq: 1, $exists: true },
				practicemanagement:req.session.adminpracticeID
	...
	.exec(function(err, paymentmanagementdata){
        if(err){
			...
			if(req.query.sSortDir_0=='desc')
			{
				...
				switch(req.query.iSortCol_0){
					// case '0': paymentmanagementdata = _.sortBy(paymentmanagementdata, '_id').reverse(); break;
					case '0': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'loanReference').reverse(); break;
					case '1': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'user.firstname').reverse(); break;
					case '2': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'user.email').reverse(); break;
					case '3': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'user.phoneNumber').reverse(); break;
					case '4': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'practicemanagement.PracticeName').reverse(); break;
					case '5': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'loanSetDate').reverse(); break;
					case '6': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'fundingTier').reverse(); break;
					case '7': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'payOffAmount').reverse(); break;
					case '8': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'maturityDate').reverse(); break;
					case '9': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'createdAt').reverse(); break;
					case '10': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'status').reverse(); break;
					case '11': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'status').reverse(); break;
					case '12': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'apr').reverse(); break;
					case '13': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'approvedunderwriter').reverse(); break;
					default: break;
    			};
			}
			else
			{
				switch(req.query.iSortCol_0){
					//case '0': paymentmanagementdata = _.sortBy(paymentmanagementdata, '_id'); break;
					case '0': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'loanReference'); break;
					case '1': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'user.firstname'); break;
					case '2': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'user.email'); break;
					case '3': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'user.phoneNumber'); break;
					case '4': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'practicemanagement.PracticeName'); break;
					case '5': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'loanSetDate'); break;
					case '6': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'fundingTier'); break;
					case '7': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'payOffAmount'); break;
					case '8': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'maturityDate'); break;
					case '9': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'createdAt'); break;
					case '10': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'status'); break;
					case '11': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'status'); break;
					case '12': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'apr'); break;
					case '13': paymentmanagementdata = _.sortBy(paymentmanagementdata, 'approvedunderwriter'); break;
					default: break;
    			};
			}
			...
			if(req.query.sSearch)
			{
				 ...
				 paymentmanagementdata=_.filter(paymentmanagementdata,function(item){
					...
					/* if(item.user.screenName!=null)
					 ...
					 if(item.approvedunderwriter!=null)
					 {
					 	 if(item.approvedunderwriter.toLowerCase().indexOf(search)>-1 )
						 {
							 return true;
						 }
					 }
            ...
            paymentmanagementdata.forEach(function(paydata,loopvalue){
				...
				if(paydata.approvedunderwriter!='' && paydata.approvedunderwriter!=null )
				{
					var approvedunderwriter = paydata.approvedunderwriter;
				}else{
					var approvedunderwriter = '--';
				}
				...
				if(paydata.status != "PAID OFF")
				{
					...
					_.forEach(paydata.paymentSchedule, function(payDetails) {
						...
						if(setcurrent==0)
						{
							...
							else if(payDetails.status == "OPENED" || payDetails.status == "CURRENT")
							{
			   ...
			   var practicename ='--';
				...
				/*pendinguser.push({ loopid:loopid, loanReference: payloanReference, name: payuserNameLink,directMail: directMailUser,badList: badListUser, email: payuserEmail,phoneNumber: payuserphoneNumber,payOffAmount:paydata.payOffAmount,maturityDate:paydata.maturityDate,createdAt:paydata.createdAt, status: statusicon, paymentstatus: paydata.status, paymenttype: paytype,apr:apr,approvedunderwriter:approvedunderwriter });*/
				
				pendinguser.push({ loopid:loopid, loanReference: payloanReference, name: payuserNameLink, email: payuserEmail,phoneNumber: payuserphoneNumber,practicename:practicename, procedureDate: procedureDate, payOffAmount:payOffAmount,maturityDate:paydata.maturityDate,createdAt:paydata.createdAt,
					status: statusicon, paymentstatus: paydata.status, paymenttype: paytype,apr:apr,approvedunderwriter:approvedunderwriter,fundingTier: fundingTier });
            });
...
function changeBankAction(req, res){
	...
	if(payID)
	{
		...
		.populate('user')
		.then(function(paymentmanagementdata){
		    var useremail = 	paymentmanagementdata.user.email;
			var userDetails = 	paymentmanagementdata.user;

			var changebankToken = Math.random().toString(36).slice(-10);
			paymentmanagementdata.changebankToken = changebankToken;
			paymentmanagementdata.save(function(err) {
				if (err) {
				 	req.session.bankerror='';
		    		req.session.bankerror = 'Unable to send change bank link. Try again!';
		    		return res.redirect("/admin/getAchUserDetails/"+payID);
				}

		    	EmailService.changeBankAccount(userDetails,payID,changebankToken);
				var userreq={};
				var usersubject = 'Change Bank';
				var userdescription = 'Change Bank email';
				userreq.userid = userDetails.id;
				userreq.logdata= 'Change Bank'+useremail;
				Useractivity.createUserActivity(userreq,usersubject,userdescription);

				req.session.successmsg='';
				req.session.successmsg = 'Change bank link has been sent successfully.';
				req.session.banksuccessmsg='';
...
/*function repullPlaidInfo(req, res) {
    ...
    var userbankaccountID = req.param('userbankaccountID');
	var accountID = req.param('accountID');

	if(userbankaccountID && accountID )
	{
		var mainBalance={};
		var mainplaidMeta={};
							...
							.then(function (transactionDetails) {
									...
									.then(function (userInfo) {
											...
											_.forEach(accountDetails.accounts,function(accountsval){
												...
												{
													var mainBalance = accountsval.balances;
													var mainplaidMeta={
												...
												if (i == forlength) {
													...
													.then(function (entity) {
														...
														.then(function(repullDetails) {
															...
															_.forEach(repullDetails, function(repullaccountvalue) {
																 repullaccountvalue.createdAt = moment(repullaccountvalue.createdAt).format("LL");
																 if(repullaccountvalue.numbers)	{
																		 var repullstr = repullaccountvalue.numbers.account;
																		 if ("undefined" !== typeof repullstr && repullstr!='' && repullstr!=null){
																				repullstr.numbers.account = repullstr.replace(/\d(?=\d{4})/g, "X");
																		 }
																	}
															})
															...
															//var json = {
//																status: 200,
//																message:'Plaid information fetched successfully',
//																repullDetails: repullDetails,
//																repullCount: repullDetails.length,
//															};
//															res.contentType('application/json');
//															res.json(json);
																...
																res.render("admin/pendingach/repullplaidinfo" , {  repullDetails:repullDetails, repullCount:repullDetails.length}, function(err, listdata){
									...
									.catch(function (err) {
										 sails.log.error("Plaid user information error::",err);
										 var json = {
											status: 400,
...
function repullPlaidInfo(req, res) {
  ...
  var userbankaccountID = req.param('userbankaccountID');
	...
	//sails.log.info("userbankaccountID::",userbankaccountID);
			...
			.then(function(userbankaccount){
				...
				if(userbankaccount)
				{
					...
					.then(function (accountDetails) {
							...
							InstitutionService.getAssetTransactionDetail(token,accountDetails, "all", accountData.user )
							.then(function (transactionDetails) {

									//sails.log.info("transactionDetails::",transactionDetails);
									...
									.then(function (userInfo) {*/
											...
											//sails.log.info("userInfo::",userInfo);
											 ...
											 //sails.log.info("total_transactions::",transactionDetails.total_transactions);
											 ...
											 if(transactionDetails.total_transactions>0)
											 {
												...
												_.forEach(transactionDetails.transactions,function(transaction){
													...
													if(!transactionGenerated.hasOwnProperty(transaction.account_id))
													{
														//sails.log.info("Enter if condition::");
														transactionGenerated[transaction.account_id]=[];
													}
													else
													{
													...
													//sails.log.info("transaction obj data::", transobj);
											...
											_.forEach(accountDetails.accounts,function(accountsval){
												...
												if(accountDetails.numbers && accountDetails.numbers.ach){
													filteredAccountsDetails = _.filter(accountDetails.numbers.ach, { "account_id": accountsval.account_id });
												}
											    //-- ReferenceError: filteredAccountsDetailsJson is not defined (Fixed on oct31, 2018)
												let filteredAccountsDetailsJson = {};
												if (filteredAccountsDetails.length >0) {
													 filteredAccountsDetailsJson = filteredAccountsDetails[0];
												}

													sails.log.info("accountNumber::",accountData.accountNumber);
												...
												//sails.log.info("accountsData::",accountsData);
											  //sails.log.info("Final mainBalance::",mainBalance);
												//sails.log.info("Final mainplaidMeta::",mainplaidMeta);
													...
													.then(function (entity) {
														...
														.then(function(repullDetails) {
															...
															_.forEach(repullDetails, function(repullaccountvalue) {
																...
																res.render("admin/pendingach/repullplaidinfo" , {  repullDetails:repullDetails, repullCount:repullDetails.length,institutionName: institutionName}, function(err, listdata){
															...
															UserConsent.reGenerateLendingDisclosureAgreement(paymentManagement.id,res,req)
																.then(function (lendingreponse) {
																	...
																	var loanData = {
																		...
																		"practiceId": paymentmanagementdata.user.practicemanagement
																	};
...
function ajaxOpenApplicationAchAction(req,res){
	...
	sails.log.info("sorttype:",sorttype);
	sails.log.info("sorttypevalue:",sorttypevalue);*/
						  ...
						  if(screenDetails.length>0)
						  {
							  ...
							  if(req.query.sSortDir_0=='desc')
							  {
								  ...
								  switch(req.query.iSortCol_0){
									  ...
									  case '1': screenDetails = _.sortBy(screenDetails, 'applicationReference').reverse(); break;
									  case '2': screenDetails = _.sortBy(screenDetails, 'paymentdata.loanReference').reverse(); break;
									  ...
									  case '8': screenDetails = _.sortBy(screenDetails, 'creditscore').reverse(); break;
									  case '10': screenDetails = _.sortBy(screenDetails, 'fundingTier').reverse(); break;
									  case '11': screenDetails = _.sortBy(screenDetails, 'createdAt').reverse(); break;
									  case '12': screenDetails = _.sortBy(screenDetails, 'lastScreenName').reverse(); break;
									  case '15': screenDetails = _.sortBy(screenDetails, 'userdata.registeredtype').reverse(); break;
									  case '16': screenDetails = _.sortBy(screenDetails, 'userdata.underwriter').reverse(); break;
									  default: break;
								  };
							  }
							  else
							  {
								  switch(req.query.iSortCol_0){
									  ...
									  case '2': screenDetails = _.sortBy(screenDetails, 'paymentdata.loanReference'); break;
									  ...
									  case '10': screenDetails = _.sortBy(screenDetails, 'fundingTier'); break;
									  case '11': screenDetails = _.sortBy(screenDetails, 'createdAt'); break;
									  case '12': screenDetails = _.sortBy(screenDetails, 'lastScreenName'); break;
									  case '15': screenDetails = _.sortBy(screenDetails, 'userdata.registeredtype'); break;
									  case '16': screenDetails = _.sortBy(screenDetails, 'userdata.underwriter'); break;
									  default: break;
								  };
							  }
							  ...
							  PracticeManagement.find()
						  	  .then(function (practiceData){
									...
									screenDetails.forEach(function(screentrackingdata,loopvalue){
										...
										var creditScore = "0";
										var apr ='--';
										var payOffAmountValue  = 	'0';
										var payloanReference='--';
										var systemUniqueKeyURL;
										var applicationType='--';

									    var userinfo = screentrackingdata.userdata;
										var paymentdetails = screentrackingdata.paymentdata;
										...
										if(paymentdetails.length>0)
										{
											...
											if((screentrackingdata.iscompleted === 0 && paydata.achstatus === 4) || (screentrackingdata.iscompleted === 1 && [0,1].indexOf(paydata.achstatus) >= 0)) {
												promissoryNoteSign = "Yes";
											}
											...
											if(paydata.payOffAmount) {
												payOffAmountValue = parseFloat(paydata.payOffAmount);
											}
										...
										else
										{
											...
											{
												if(screentrackingdata.offerdata[0].length>0)
												{
													var offerData = screentrackingdata.offerdata[0];
											...
											{
												payOffAmountValue = screentrackingdata.requestedLoanAmount;
											}
										}
										var fundingTier = "--";
